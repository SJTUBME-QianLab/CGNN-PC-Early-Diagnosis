...
class Patch_Dataset_orisample(Dataset):
    def __init__(self, config, args, partition, mode):  # image_names=partition['train']
        ...
        self.idx = []
        self.image_mat = {}
        image_location = generate_patch_location(partition[mode], self.image_path, mode, patch_size, stride, max_amount=1000)
...
def read_image_name(config, args, logging):
    ...
    return partition, num_parts_h_renji, num_parts_t_renji # [[xx …… xx],……] [xx,xx,xx] [xx,xx,xx]
...
def generate_patch_location(image_list, save_path, mode, patch_size=50, stride=25, max_amount=1000):
